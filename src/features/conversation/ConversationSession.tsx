...
export const ConversationSession: React.FC<ConversationSessionProps> = ({
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 animate-fade-in flex flex-col h-[calc(100vh-80px)]">
      {/* Session Top Bar */}
      <div className="bg-white border border-slate-200/80 rounded-xl px-5 py-3.5 mb-4 shadow-2xs flex items-center justify-between gap-4 shrink-0">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-base font-bold text-slate-900">
              {scenario.title}
            </h2>
            <span className="px-2 py-0.5 rounded bg-indigo-50 border border-indigo-100 text-indigo-700 text-xs font-bold">
              {level}
            </span>
          </div>
      ...
      {/* Messages Stream */}
      <div className="flex-1 overflow-y-auto bg-slate-50 border border-slate-200/60 rounded-2xl p-4 sm:p-6 space-y-4 mb-4">
          ...
          return (
            <div key={turn.id} className={`flex flex-col ${isUser ? 'items-end' : 'items-start'}`}>
              ...
              >
                ...
                {!isUser && (
                  <button
                    ...
                    className="mt-2 text-2xs font-bold text-indigo-600 hover:text-indigo-800 transition-colors cursor-pointer inline-block"
                  >
                    Play response
                  </button>
                )}
              ...
              {turn.feedback?.hasCorrection && (
                <div className="mt-2 max-w-[85%] bg-amber-50 border border-amber-200/80 rounded-xl p-3.5 text-xs text-amber-900 shadow-2xs">
        ...
        {isLoading && (
          <div className="flex flex-col items-start">
            ...
            <div className="bg-white border border-slate-200/80 rounded-2xl rounded-bl-xs px-5 py-3 text-xs text-slate-500 italic shadow-2xs">
              Thinking and typing response...
            </div>
          </div>
        )}
      ...
      {/* Static Hint Drawer */}
      {showHint && (
        <div className="bg-indigo-50/80 border border-indigo-200 rounded-xl p-3.5 mb-3 text-xs text-indigo-900 animate-fade-in shrink-0">
      ...
      {/* Input Area */}
      <div className="bg-white border border-slate-200/80 rounded-2xl p-3.5 shadow-xs shrink-0">
        <textarea
          ref={textareaRef}
          value={inputText}
          ...
          disabled={isLoading}
          rows={2}
          className="w-full resize-none border-0 bg-transparent text-sm text-slate-900 placeholder:text-slate-400 focus:outline-hidden disabled:opacity-50"
        />
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-100 text-xs">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-200 disabled:text-slate-400 text-white text-xs font-bold transition-all cursor-pointer disabled:cursor-not-allowed"
            >
